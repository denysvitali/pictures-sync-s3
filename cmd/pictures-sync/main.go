...
func handleCardRemoved(event sdmonitor.Event, stateMgr *state.Manager) {
	log.Printf("SD card removed: %s", event.DevName)
	stateMgr.SetSDCard(false, "")

	// If we were syncing, mark as error
	currentState := stateMgr.GetState()
	if currentState.Status == state.StatusSyncing {
		log.Println("Sync interrupted by card removal")
		stateMgr.FinishSync(false, nil)
	}
