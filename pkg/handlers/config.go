...
//
// Only structural/non-identifier fields belong here. Keys that carry account
// identifiers, credentials, access keys, or user-specific values must NOT be
// listed — they are redacted by default.
var safeRcloneConfigKeys = map[string]bool{
	...
	"scope":                  true,
	"env_auth":               true,
	// Omitted (identifier/credential fields — redacted for safety):
	//   account, account_id, key_id, user, username, host, port, url,
	//   download_url, shared_credentials_file, profile, team_drive,
	//   root_folder_id, vendor
}
