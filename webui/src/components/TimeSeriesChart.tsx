...
const VIEW_W = 800
const PAD_LEFT_BASE = 50
const PAD_RIGHT_BASE = 16
const PAD_TOP = 12
const PAD_BOTTOM = 32
...
export function TimeSeriesChart({
    ...
    return series
      .map((s): SeriesRender | null => {
        ...
        if (validPts.length > 1) {
          ...
          areaD = `M ${validPts[0]!.x} ${baselineY} `
          for (const p of validPts) areaD += `L ${p.x} ${p.y} `
          areaD += `L ${validPts[validPts.length - 1]!.x} ${baselineY} Z`
        }
        return {
          key: s.key,
          label: s.label,
  ...
  const ariaLabel = useMemo(() => {
    ...
    return `Time series chart showing ${series.map((s) => s.label).join(', ')}`
  }, [series])

  if (!Array.isArray(data) || data.length === 0 || series.length === 0) {
    return (
      <div
        className="flex items-center justify-center rounded-lg bg-surface-800/40 text-sm text-surface-500"
        style={{ height }}
        role="img"
  ...
  if (hoverPoint && scales) {
    ...
    if (tooltipX + 180 > padLeft + chartW) tooltipFlip = true
  }

  const tooltipW = 180
  const tooltipH = 28 + series.length * 16
  ...
  return (
    <div className="relative text-surface-200">
      ...
      >
        ...
        {seriesRender.map((s) => (
          <g key={s.key}>
            {s.areaD && <path d={s.areaD} fill={s.fillColor} fillOpacity={0.12} />}
            {s.pathD && (
              <path
                d={s.pathD}
                fill="none"
        ...
        {hoverPoint && scales && hoverIdx !== null && (
          <g pointerEvents="none">
            ...
            {seriesRender.map((s) => {
              ...
              if (!p) return null
              return <circle key={`hp-${s.key}`} cx={p.x} cy={p.y} r={4} fill={s.color} stroke="#0f172a" strokeWidth={1.5} />
            })}
            ...
            >
              ...
              />
              <text x={10} y={16} fontSize={10} fill="rgba(255,255,255,0.65)">
              ...
              {seriesRender.map((s, i) => {
                ...
                return (
                  <g key={`tt-${s.key}`} transform={`translate(10, ${30 + i * 16})`}>
                    <rect x={0} y={-8} width={8} height={8} fill={s.color} rx={2} />
                    <text x={14} y={0} fontSize={11} fill="rgba(255,255,255,0.9)">
                      {s.label}
                    </text>
                    <text
                      x={tooltipW - 20}
      ...
      {zoom && (
        <button
          type="button"
          onClick={resetZoom}
          className="absolute right-2 top-2 rounded-md bg-surface-800/80 px-2 py-1 text-xs font-medium text-surface-200 ring-1 ring-white/10 hover:bg-surface-700"
        >
          Reset zoom
        </button>
      )}
      ...
      {series.length > 1 && (
        <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-surface-300">
          {series.map((s) => (
